...
const { incrementExecute } = require('./adminController');
...
function b64e(str) { return Buffer.from(str || '').toString('base64'); }
function b64d(str) { return str ? Buffer.from(str, 'base64').toString('utf-8') : ''; }
...
function buildPythonHarness(userCode, funcName, cases, opts = {}) {
...
function buildJSHarness(userCode, funcName, cases, opts = {}) {
  const { linkedList, binaryTree } = opts;
  let helpers = '';
  let preprocess = '';
  let postprocess = '';
...
const executeCode = async (req, res) => {
  ...
  try {
    ...
    if (compileOut) {
      return res.json({ type: 'raw', stdout: '', stderr: compileOut });
    }
    ...
    if (hasTestHarness && stdout.trim()) {
      try {
        ...
        if (Array.isArray(testResults)) {
          const passed = testResults.filter(r => r.p).length;
          const enriched = testResults.map((r, i) => ({
            ...r,
            isEdgeCase: !!(testData.cases[i] && testData.cases[i].isEdgeCase),
          }));
          return res.json({ type: 'test', testResults: enriched, passed, total: enriched.length, stderr });
        }
      } catch (_) {}
    }

    return res.json({ type: 'raw', stdout, stderr });

  } catch (err) {
    console.error('Execute error:', err.message);
    return res.status(503).json({ error: 'Code execution unavailable. Try again in a moment.' });
  }
};
